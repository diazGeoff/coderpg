...
<div class="form-group fieldcontain ${hasErrors(bean: missionInstance, field: 'quest', 'error')} required">
	...
	<g:select id="quest" name="quest.id" optionValue="name" from="${com.coderpg.Quest.list()}" optionKey="id" required="" value="${missionInstance?.quest?.id}" class="many-to-one form-control"/>
</div>
