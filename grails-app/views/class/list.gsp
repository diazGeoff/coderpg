			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="description" title="${message(code: 'class.description.label', default: 'Description')}" />
				...
				<g:each in="${classInstanceList}" status="i" var="classInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${classInstance.id}">${fieldValue(bean: classInstance, field: "name")}</g:link></td>
					
						<td>${fieldValue(bean: classInstance, field: "description")}</td>
